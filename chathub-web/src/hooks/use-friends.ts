...
export function useFriends(userId: number | null, token: string | null) {
  ...
  const acceptFriendRequestHook = useCallback(async (values: FriendshipRequest): Promise<SuccessResponse | null> => {
    if (!token) {
  ...
  const unsentFriendRequestHook = useCallback(async (friendId: number): Promise<SuccessResponse | null> => {
    if (!userId || !token) {
  ...
  const sendFriendRequestHook = useCallback(async (data: FriendshipRequest): Promise<SuccessResponse | null> => {
    if (!token) {
      setError("Authentication token is missing.");
      return null;
    }
